<template>
  ...
  <baidu-map class="bm-view" center="河北" @ready="handler" :scroll-wheel-zoom="true" >
    ...
    <!--state 不为3和4就是离线  3静止 4运动。  num   0离线   超过0在线-->
    <bml-marker-clusterer :averageCenter="true" :styles= 'styles'>
       <bm-marker 
          v-for="(marker,key) of markers" 
          :key="key" 
          :position="{lng: marker.longitude, lat: marker.latitude}"
          :icon="{url: (marker.state!=3||marker.state!=4)&&(marker.num==0||!marker.num) ? '/img/ic_task_off_offline.png' : 
                      (marker.state!=3||marker.state!=4)&&marker.num>0 ? '/img/ic_task_on_offline.png' : 
                      marker.state==3&&(marker.num==0||!marker.num) ? '/img/ic_task_off_static.png' : 
                      marker.state==3&&marker.num>0 ? '/img/ic_task_on_static.png' : 
                      marker.state==4&&(marker.num==0||!marker.num) ? '/img/ic_task_off__drivin.png' : 
                      '/img/ic_task_on_static.png', size: {width: 30, height: 30}}"
...
export default {
  ...
  mounted () {
    ...
    this.styles = [
      { 
        url: '../../../assets/monitoring/ic_aggregation.png', 
