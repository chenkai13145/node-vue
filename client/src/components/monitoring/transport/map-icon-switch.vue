...
* @module c:\SpeedinessDocument\vuesys\src\components\monitoring\transport\map-icon-switch.vue
* @description 地图图标选择
* @author wind-lc
* @date 2019-05-15
* @param {String} defaultMapTab  默认选中项
* @return {Event} mapTab   返回选中的地图类型名字
* @example <MapIconSwitch :defaultMapTab="defaultMapTab" @mapTab="mapTab"></MapIconSwitch>
*/
-->
<template>
  <div>
    <ul class="map-icon-switch"
        :class="{'map-icon-switch-full-screen': false}">
    ...
    handleSwitch (index) {
      ...
      this.mapIcon.forEach((item, i) => {
        if (i === index) {
          ...
          this.$emit('mapTab', _this.coun, this.searcgVal)
        } else {
          item.active = false
        }
      })
